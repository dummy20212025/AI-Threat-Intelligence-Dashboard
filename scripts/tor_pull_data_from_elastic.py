...
def fetch_tor_nodes(running=True):
    ...
    for relay in data.get("relays", []):
        ip = relay["or_addresses"][0].split(":")[0] if relay["or_addresses"] else None
        nodes.append({
...
def parallel_resolve(domains, max_workers=100):
    results = [None] * len(domains)

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(resolvable, domain): idx for idx, domain in enumerate(domains)}

        for future in tqdm(as_completed(futures), total=len(domains), desc="Resolving"):
            idx = futures[future]
            try:
                results[idx] = future.result()
            except Exception:
                results[idx] = "IP Not Found"

    return results

async def is_hosted_async(domain, resolver):
    try:
...
#     dff=pd.DataFrame(df['sslServerName'])
#     dff=dff.drop_duplicates()
#     dff['Valid']=dff['sslServerName'].apply(tld_valid_check)
#     dff = dff[dff['Valid']=='Yes']
#     #print("Resolving IPs")
#     dff['Resolve'] = await async_check(dff['sslServerName'].tolist())
#     # dff['Resolve'] = parallel_resolve(dff['sslServerName'].tolist())
#     dff = dff[dff['Resolve']==False]
#     #print("Number of unresolved domains : ",dff.shape[0])
#     filtered_df = df[df['sslServerName'].isin(dff['sslServerName'])]
#     filtered_df=filtered_df.drop_duplicates()
#     # filtered_df.to_csv("tor_suspect.csv",index=False)
    
#     # df['Valid']=df['sslServerName'].apply(tld_valid_check)
#     # df = df[df['Valid']=='Yes']
#     # df['Resolve'] = parallel_resolve(df['sslServerName'].tolist())
#     # df=df[df['Resolve']=='IP Not Found']
#     return filtered_df
...
def preprocess(filenames):
    ...
    #print("Resolving IPs")
    # dff['Resolve'] = await async_check(dff['sslServerName'].tolist())
    # dff['Resolve'] = asyncio.run(async_check(dff['sslServerName'].tolist()))..................
    dff['Resolve'] = parallel_resolve(dff['sslServerName'].tolist())
    dff = dff[dff['Resolve']=="IP Not Found"]
    # dff = dff[dff['Resolve']==False]
    #print("Number of unresolved domains : ",dff.shape[0])
    filtered_df = df[df['sslServerName'].isin(dff['sslServerName'])]
    filtered_df=filtered_df.drop_duplicates()
...
def get_filename(timestring,prefix='tor'):
...
def main():
    ...
    time_range = [
        ['09.02.2026 12:15:00','09.02.2026 12:30:00']
        ]
    # time_range = [
    #    date_time
    #    ]
...
if __name__=="__main__": 
    ...
    # dead_df['Status'] = "Inactive"
    # tor=pd.concat([active_df,dead_df],axis=0)
    # # tor.to_csv("TOR.csv",index=False)
    # final_df['Known_TOR'] = np.where(final_df['DstIP'].isin(tor['ip']), 'Yes', 'No')     #check if detected ip i s present in the exisitng list
    # status_lookup=tor.set_index('ip')['Status'].to_dict()
    # final_df['Status']=final_df['DstIP'].map(status_lookup).fillna("Not Found")  
    
    
    final_df.to_csv("tor.csv",index=False)
