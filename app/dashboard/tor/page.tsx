...
import {
    ...
    ShieldAlert,
    Clock,
    SearchX // Added for the empty result state
} from 'lucide-react';
...
export default function ReconShieldPage() {
    ...
    const [hasQueried, setHasQueried] = useState(false); // Track if a fetch has been attempted
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await fetch(`/api/tor?${params}`);
            ...
            Papa.parse(csvText, {
                ...
                complete: (results) => {
                    if (results.data && results.data.length > 0) {
                        ...
                        // Ensure the first row isn't just empty values
            ...
            console.error("Error fetching TOR data:", error);
            setData([]);
            setHasQueried(true);
            setLoading(false);
        }
                        ...
                        {loading ? (
                            ...
                            >
                                <div className={`p-8 rounded-full ${isDark ? "bg-purple-900/20 text-purple-400" : "bg-purple-50 text-purple-200"}`}>
                                    {hasQueried ? <SearchX size={48} /> : <ShieldAlert size={48} strokeWidth={1.5} />}
                                </div>
